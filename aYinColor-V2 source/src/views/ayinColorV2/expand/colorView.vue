...
<script setup>
//import ce from "./ayincolor-expand-vue.js"
const jscolor=(val)=>{
  return $c[val];
}
 const colors=[
...
<template>
  <div class="ayincolor-expand">
    <h1>ayin's color Expand (基于full版本改造，于2022年3月15日改造后集成到vue3版本的ayin platform)</h1>
    <p>由 ayin86@163.com 开发。包含less color 和 js color。less颜色表基于less.js开发。js颜色表基于chroma.js开发。</p>
    <p>用vue生成的整合代码，Expand版约6000多行代码。一共24种颜色。每种颜色现在分为6种饱和度，19种明暗，每种颜色114种变化。 </p>
    <p>less中使用方式如@reA01，@blF19这样的格式。re\bl等代表色相，A-F代表饱和度，数字代表明暗。</p>
    <p>拆分解释比如 @ re A 01  其中re代表色相，A代表饱和度，01代表亮度。其中饱和度有6个等级，明暗有19个等级</p>
    <p>饱和度为A-F，一共6个等级A为最艳丽的饱和度，不建议直接使用，取色建议从B或者C开始取。</p>
        ...
        <div class="group-wrapper" v-for="(item,index) in colors" :key="'g4-'+index">
          ...
          <div class="color-wrapper" :class="'color-'+item.short+'-wrapper'" >
            <div class="sub-wrapper" v-for="(item2,index2) in fullLevel" :key="'g5-'+index2">
              <p class="title-sub">{{item.short}}-{{item2}}</p>
              <ul class="color-group "><!--+colors[i]+"-"+sort[j]+""-->
                  <li :ref="'jscolor-'+item.short+item2+item3" :style="'background-color:'+jscolor(item.short+item2+item3)" v-for="(item3,index3) in lightDark" :key="'g6-'+index3">{{item2+item3}}</li><!--"+colors[i]+"-"+sort[j]+k+"-->
              </ul>
            </div>
          </div>
        </div>
...
<style lang="less">
//@import "./ayincolor-expand.less";
@import "./ayincolor-expandbg.less";
.ayincolor-expand{
