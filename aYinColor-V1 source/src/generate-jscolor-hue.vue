<template>
  <div class="generate">
      ...
      <span v-for="(item,index) in colors" :key="'g8-'+index">
        <span v-for="(item2,index2) in normal" :key="'g9-'+index2">
          <span v-for="(item3,index3) in 9" :key="'g0-'+index3">
            <span v-if="index3<5">
              {{item.short}}h{{item2}}{{item3}}:chroma(chroma({{item.short}}{{item2}}).set("hsl.h",{{index*15}}-$spin{{4-index3}})).set("hsl.l",$level{{item3}}).hex(),<br>
            </span>
            <span v-if="index3>4">
              {{item.short}}h{{item2}}{{item3}}:chroma(chroma({{item.short}}{{item2}}).set("hsl.h",{{index*15}}+$spin{{index3-4}})).set("hsl.l",$level{{item3}}).hex(),<br>
            </span>
          </span>
          <br>
        </span>
