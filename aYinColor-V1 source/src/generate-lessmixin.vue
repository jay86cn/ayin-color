<template>
  <div class="generate">
    ...
    <span v-for="(item,index) in colors" :key="'g3-'+index">
        <span v-for="(item2,index2) in normal" :key="'g4-'+index2">
          @{{item.short}}{{item2}}:saturate(@{{item.short}},@c{{item2}});
          <span v-for="(item3,index3) in 9" :key="'g5-'+index3">
      ...
      <span v-for="(item,index) in colors" :key="'g6-'+index">
        <span v-for="(item2,index2) in normal" :key="'g7-'+index2">
          <span v-for="(item3,index3) in 9" :key="'g8-'+index3">
            <span v-if="index3<5">
              @{{item.short}}{{item2}}{{item3}}:lighten(@{{item.short}}{{item2}},@level{{4-index3}});<br>
            </span>
            <span v-if="index3>4">
              @{{item.short}}{{item2}}{{item3}}:darken(@{{item.short}}{{item2}},@level{{index3-4}});<br>
            </span>
          </span>
          <br>
        </span>
