      ...
      <span v-for="(item,index) in colors" :key="'g6-'+index">
        <span v-for="(item2,index2) in normal" :key="'g7-'+index2">
          <span v-for="(item3,index3) in 9" :key="'g8-'+index3">
            <span v-if="index3<5">
              @{{item.short}}h{{item2}}{{item3}}:lighten(  spin(@{{item.short}}{{item2}},@spin{{index3+1}})  ,@level{{4-index3}});<br>
            </span>
            <span v-if="index3>4">
              @{{item.short}}h{{item2}}{{item3}}:darken(  spin(@{{item.short}}{{item2}},@spin{{index3+1}})  ,@level{{index3-4}});<br>
            </span>
          </span>
          <br>
        </span>
